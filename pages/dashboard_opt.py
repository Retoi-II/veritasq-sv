            ...
            try:
                ...
                with st.expander("Player Metadata"):
                    ...
                    if GET_WHOSCORED['game_info'].exists():
                        ...
                        @st.cache_data(show_spinner=True, show_time=True)
                        def call_read_events(ws_scraper, id):
                            try:
                                events = ws_scraper.read_events(match_id=id)
                                return events
                            except Exception as e:
                                print(f"[red]ERROR[/red]\tFailed to fetch match {id}: {e}")
                                return pd.DataFrame()
                        ...
                        MAX_PAGES_PER_INSTANCE = 1
                        pages_scraped = 0
                        ws_scraper = None

                        def get_new_scraper(league, season):
                            return sd.WhoScored(leagues=league_str, seasons=season_int, no_cache=False, no_store=False)
                            ...
                            try:
                                ...
                                for j, id in enumerate(club_games, start=1):
                                    ...
                                    if not file_name.exists():
                                        ...
                                        if ws_scraper is None or pages_scraped >= MAX_PAGES_PER_INSTANCE:
                                            if ws_scraper is not None and hasattr(ws_scraper, '_driver'):
                                                ws_scraper._driver.quit()

                                            print("[blue]INFO[/blue]\tStarting fresh WhoScored browser session...")
                                            ws_scraper = get_new_scraper[league_str, season_int]
                                            pages_scraped = 0
                                    ...
                                    # ---------- ( OFFLINE LOAD MODE ) ---------- #
                                        ...
                                        # ---------- ( RECOVERY MODE ) ---------- #
                                            ...
                                            events_fix = call_read_events(ws_scraper, id)
                                            pages_scraped += 1
